...
def index():
    ...
    if form.validate_on_submit():
        equivalentes = Equivalente.query.\
            ...
            all()
        # equivalentes = Equivalente.query.\
        #     filter(Equivalente.nombre.contains("soya")).\
        #     all()        
...
def nuevo_equivalente():
    ...
    if form.validate_on_submit():
        ...
        propiedades = [Propiedad(concepto_id=c.id, valor="N.D.")
                       for c in conceptos]
...
def delete_equivalente(index):
    ...
    propiedades = db.session.query(Propiedad).filter(
        Propiedad.equivalente_id == index).all()
    for p in propiedades:
        db.session.delete(p)
    eq = \
...
def edit_prop(index):
    ...
    if form.validate_on_submit():
        prop = db.session.query(Propiedad).filter(
            Propiedad.id == index).first()
